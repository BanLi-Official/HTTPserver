...
{
    struct EpollInfo *info = (struct EpollInfo *)arg;
    if (info->fd == -1)
    {
    ...
    int size = sizeof(add);
    int cfd = accept(info->fd, (struct sockaddr *)&add, (socklen_t *)&size);
    ...
    res = epoll_ctl(info->epfd, EPOLL_CTL_ADD, cfd, &ev);
    if (res == -1)
    {
...
void* rescvHTTPRequest(void *arg)
{
    struct EpollInfo *info = (struct EpollInfo *)arg;
    if (info->fd == -1 || info->epfd == -1)
    {
    ...
    int len;
    int total = 0;
    while ((len = recv(info->fd, temp, sizeof temp, 0)) > 0) // 边沿epoll导致需要将其不断循环读出来，在这里不需要完全存入缓冲区也可以，因为只需要http 请求的头一行
    {
    ...
    if (len == -1 && errno == EAGAIN)
    {
        ...
        parseHTTPRequest(buf, info->fd); // 这里后期要修改为buf内容
    }
    else if (len == 0)
    {
...
int epollRun(int lfd)
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            struct EpollInfo *info =(struct EpollInfo *)malloc(sizeof(struct EpollInfo));
            if (events[i].data.fd == lfd)
            {
                // 连接客户端
                info->fd=events[i].data.fd ;
                info->epfd = epfd;
                pthread_create(&info->pid , NULL , &connectClient , info);
            }
            else
            {
                // 客户端的数据交流
                info->fd=events[i].data.fd ;
                info->epfd = epfd;
                pthread_create(&info->pid , NULL , &rescvHTTPRequest , info);
            }
        }
    }
